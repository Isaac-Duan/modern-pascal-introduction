...
## 前言
市面上已经有不少介绍Pascal语言的书籍了，但是很多只是讲解传统Pascal语言，关于类、单元(模块)、泛型讲解甚少。有些书籍甚至成书于Pascal引入面向对象语法之前，有些书籍虽然讲解了Delphi，但是对于现代特性比如泛型却没有涉及。
所以我要写一本关于* 现代Object Pascal *的快速入门书籍，重点讲解他的面向对象部分。
...
这就是第一个入门的程序，你可以 _编译_ 和 _执行_ 他。

* 如果你在命令行中使用FPC，那你需要新建一个文件命名为 `myprogram.lpr` 然后执行命令 `fpc myprogram.lpr`.
* 如果你使用 _Lazarus_, 则需要新建一个工程 (从菜单上执行 _Project_ -> _New Project_ -> _Simple Program_). 保存为 `myprogram` 并将上述代码粘贴入主文件中， 在菜单中执行 _Run -> Compile_.
* 然后你就得到一个命令行的程序，你可以在命令行中编译并执行他。

本书其他本分将重点介绍Object Pascal语言本身。
...
----
通过对内置变量 `Result` 赋值可以设置函数的返回值。你也可以像对待局部变量那样任意使用变量 `Result`。
[source,pascal]
----
...
----
你也可以用函数名来设置返回值(就像上面的例子中的 `MyFunction`)，但是我不建议那样做。
你可以递归调用函数自身。如果你递归调用无参数函数，请显示地在函数名后面跟上括号 `()` 。这样Pascal就知道你是调用函数，而不是取出当前的返回值。例如:
[source,pascal]
----
...
----
函数和过程在遇到 `end;` 后会结束调用，你可以使用 `Exit` 来提前结束函数或过程的执行。如果在函数中使用无参数的 `Exit`，函数将返回最后设置的 `Result` 值。你也可以使用 `Exit(X)` 来设置返回值为X并 *立即* 结束当前函数的执行。
...
----
注意：函数的返回值可以被丢弃，函数也可以像过程那样被使用。这可能使得函数有一些计算返回值以外功能的 _副作用_(比如修改全局变量)。例如：
[source,pascal]
----
var
  Count: Integer;
...
begin
  Count := 10;
  CountMe; // 函数被调用了，但是返回值被丢弃了
  MyCount := CountMe; // 使用了函数的返回值
end.
----

### 条件判断语句 (if)

使用 `if .. then` 或者 `if .. then .. else` 来指定只有条件满足才会执行的语句。无需像C语言那样把条件语句放入括号中。
...
----
上面代码中嵌套的 `if` 是合法的。这种情况下通常将内嵌的 `if` 放入 `begin` ... `end` 语句块中，这样代码看起来更易读。下面是改进的写法，即使你添加或删除一些 `else` 声明，哪些语句会被执行也是一目了然。
[source,pascal]
----
if A <> 0 then
begin
...
### 逻辑运算符、关系运算符、位运算符
_逻辑运算符_ 有 `and`、 `or`、 `not`、 `xor` 。他们接受参数和返回值都是 _boolean_ 。当接受的参数都是integer(整型)的时候他们会按照位运算并返回integer 类型结果。
_关系运算符_ 有 `=` , `<>` , `>`, `<`, `\<=`, `>=` 。注意，Pascal中使用单个 `=` 来判断相等，赋值使用 `;=`。
_位运算符 比 关系运算符 有更高的优先级_ 。某些情况下你可能需要用括号来达到你想要的运算顺序。
下面的例子会引发一个编译错误:
...
var
  A, B: Integer;
begin
  if A = 0 and B <> 0 then ... // 错误示范
----

上面的例子无法通过编译，因为编译器会优先进行位运算符 `and` ，计算 `(0 and B)` 并返回一个integer 类型的值。然后编译器去计算 `=` ，返回一个 `boolean` 类型的结果： `A = (0 and B)`。最终 _"type mismatch"_ 错误(类型不匹配)被引发。
下面是一个正确的示例：
...
*  `MyFunction(X)` 会被先计算。
* 如果 `MyFunction(X)` 返回了 `false`, 那么整个表达式的值都清楚了 ( `false and 任何值` 结果都是 `false`), 并且 `MyOtherFunction(Y)` 也不会被计算了。
* 类似的规则适用于 `or` 。 如果第一个表达式的值是 `true` ，第二个表达式的值也不会被计算。
* 在类似是下面的例子中这条规则很有用：
+
[source,pascal]
----
...
----
+
上面的例子中即使 `A` 是 `nil` 也可以编译运行。  关键字 `nil` 是一个值为零的指针，通常也被叫做 _空指针_ (null pointer)。
...
如果一个表达式具有多个值，并且各个值决定不同的执行流程，那么使用 `case .. of .. end` 声明语句会很方便。
...
`else` 声明部分是可选的 (对应于C语言中的 `default`)。 如果所有的情况都没有匹配，而且也没有 `else`声明，那么将不会执行任何语句。

如果你很熟悉C语言，比较一下其中的 `switch` ， 你会发现C语言没有自动 _跳过机制_。这是Pascal故意设计的特性，你也不需要去手动地放置 `break` 。 任何时候,  `case`分支下的语句 _至多被执行一次_ 。

### 枚举类型(Enumerated)、 有顺序类型(ordinal types)、集合类型(sets)、固定长度数组(constant-length arrays)
...
命名约定枚举的成员应该使用类型名称的双字母缩写作为前缀，比如本例中的 `ak` 取自 _"Animal Kind"_ 两个首字母。 这是一个十分有用的约定, 尤其是枚举成员的定义都在unit中，这时枚举成员都是全局命名空间中。 添加前缀可以减少命名冲突。

注意: 命名冲突不是一个非常严重的障碍。不同的unit中定义相同的标识符不会造成命名冲突。但是尽量避免命名冲突仍有必要，尤其是在增强代码可读性和检索中尤为重要。

注意: 你也可以使用编译选项 ` {$scopedenums on} ` 来避免将枚举成员名称暴露为全局名称。但这样你就必须使用类型名称来索引枚举成员, 比如 ` TAnimalKind.akDuck `. 这样的话枚举成员名称加前缀 `ak ` 就不是那么必要了。

枚举类型是一种 _不透明_ 类型 其中的成员是自动赋值的而且都是integer类型，你无法手动为他们赋值。当然,在特殊情况下，你可以使用 `Ord(MyAnimalKind) ` 把他们转化为整型，或者使用类型转换 ` TAnimalKind(MyInteger) ` 把整型转成枚举类型。 在接下来的例子里，我们首先要确认 ` MyInteger ` 是否在序列 (0 to ` Ord(High(TAnimalKind)) ` )中。
...
1. 循环条件相反. 对于 `while .. do` 你指定的是 _什么条件下继续执行循环_, 对于 `repeat .. until` 你指定的是 _什么时候停止循环_。
2. 对于 `repeat`, _循环条件不是在一开始被检查_ ，所以 `repeat` 会至少执行一次。

* `for I := ...` 循环 * :

`for I := .. to .. do ...` 看起来很像 C语言的 `for` 循环。 但是他有更严格的约束, 比如你不能任意地控制循环的迭代元素。for 循环迭代对象必须是一组连续的数字或者有序类型。唯一可自主选择的是你可以使用 `downto` 替代 `to`来从大到小地遍历迭代序列。

正因为如此，For 循环不仅有良好的可读性，而且编译器很方便做优化。 _For 循环的上界和下界只会被计算一次_, 而且是在一开始就被计算了。
需要注意的是这个形式中的计数变量 (`I` in this example) 再循环结束后就变成了 _ (未定义)undefined_ 。 循环结束后访问 `I` 变量会引发一个编译警告， _除非_ 你使用 `Break` 或 `Exit` 来提前结束循环：这时，计数变量将会保存最后被设置的值。

* `for I in ...` 循环 *:

 `for I in .. do ..` 和很多现代语言中的 `foreach` 看起来类似。它适用于多种类型:

* 它可以遍历数组中的所有元素 (比如前面的例子).
* 它还可以遍历枚举类型中的所有可能值:
+
[source,pascal]
----
...
----
* 它可以用来遍历自定义的序列类型、泛型。And it works on custom list types, generic or not, 比如 `TObjectList` 或 `TFPGObjectList`.
+
[source,pascal]
----
...
----
+
当我们接触类(class)的概念后最后一个例子就能理解了，现在可以先记住这个例子。

### 输出(Output), 日志(logging)

在Pascal中输出一个字串可以使用 `Write` or `WriteLn` 过程.  `WriteLn` 将会在字串末尾自动加上换行.

这是一个 "神奇的" 过程. 它可以接受不定个数的参数，参数类型也可以是多种类型. 参数最终都被转化成字符串显示出来，还可以使用特定格式指定字串的对齐和数字显示的精度。
...
如果要在字符串中显示地插入换行，使用 `LineEnding` (包含在 FPC RTL). (The _Castle Game Engine_ defines also a shorter `NL` constant.) Pascal不会把反斜线组合字串转义, 所以下面的写法不会像你想象的那样执行：
...
这种只能在 _命令行应用_ 中起效果. 在你的主程序文件中确保设置编译指令 ` {$apptype CONSOLE} ` ( * 不是 * ` {$apptype GUI} ` ) . 某些平台这个指令设置与否不会影响 (比如Unix), 但是在某些平台 (比如Windows)上在GUI 应用中使用WriteLn会引发错误.
...
要把一个数字类型转成字符串类型(不仅仅是在屏幕上输出他们)，有很多方法。
* 你可以使用内置的一些函数(比如： `IntToStr` 、 `FloatToStr` )把特定类型的变量转成字符串，你还可以使用 `+` 运算符把两个字符串连接起来。例如可以这样创建字符串：  `'My int number is ' + IntToStr(MyInt) + ', and the value of Pi is ' + FloatToStr(Pi)` 。
** _优点一_: 使用非常灵活。 系统内置了很多形如 `XxxToStr` 函数及其他类似函数 (比如 `FormatFloat`), 可以满足多种类型转换需求.
// 在格式化字串方式上有很大的灵活性.
** _优点二_: 如果要使用相反的功能， 比如把一个字符串转换成 `integer` 或 `float`，你可以很容易地知道要使用 `StrToInt`, `StrToFloat` 以及其他类似函数 (比如 `StrToIntDef`) .
** _缺点_: 带有太多的 `XxxToStr` ，代码看起来不美观。
//对于class 类型，可以重载 `TObject.ToString` 方法。
//使用类型转换函数不如 `Format` 函数那样具有清晰的 _模式参数分离_ 的形式。

* `Format` 使用形式为 `Format('%d %f %s', [MyInt, MyFloat, MyString])`. 这种和C语言中的 `sprintf` 类似. 这种模式会把参数插入占位符. 占位符本身可以使用特殊语法来指定字符串的格式, 例如 `%.4f` 表示一个浮点型数值，小数点后面有4位小数.
** _优点一_: 字串 _格式_ 与 _参数_ 的分离使得代码看起来清晰。 如果你仅仅只想改变字串的显示方式而不用更改参数的时候就很方便。
** _优点二_: 无需特殊编译魔法. 你可以使用类似的语法来传递任意数量的参数到你的自己定义的过程中(将参数声明为 `array of const`). 参数列表可以被包装到[]中传入，然后可以在解包参数列表使用每个参数。You can then pass these arguments downward to `Format`, or deconstruct the list of parameters and do anything you like with them.
** _缺点_: 编译器不会检查模式与参数是否匹配，如果使用了与参数不兼容的占位符，会引发运行时错误 ( `EConvertError` 异常, 而不是段错误).
...
* `WriteStr(TargetString, ...)` 过程类似于 `Write(...)`, 不同点在于前者将结果保存在 `TargetString`.
** _优点_: 支持 `Write`的所有特性, 包括字串的格式化方式，比如 `Pi:1:4`.
** _缺点_: 字符串格式化方式是一种 "编译魔法", 是针对这些过程的特有实现. 这种方式会带来一些麻烦，比如你无法实现自己的类似功能的 `MyStringFormatter(...)` 过程. 正因为如此(另外很长时间内主流Pascal 编译器都没有实现这个过程)这个方法不是非常流行。

## 单元(Units)

单元允许你把一组操作打包到一个文件中，这样别人就能在自己的程序中使用你的单元。这就相当于其他语言中的 _模块_ 、 _包_。单元含有 接口(interface)区域(在接口区域你可以定义暴露给其他开发者使用的)和 实现区域(implementation)。单元文件名应该是单元名小写，用 .pas 作为扩展名。
...
主体程序部分应当放在名称如 `myprogram.lpr` 的文件中 (`lpr` 是 Lazarus 的主程序文件; 对于 Delphi 是 `.dpr`). 还有一些其他的命名方式，比如有些编译器直接使用 `.pas` 作为主程序文件, 有些使用 `.pp` 作为单元文件或主程序文件. 我建议使用 `.pas` 作为单元文件， `.lpr` 作为FPC/Lazarus的主程序文件.
...
一个单元文件也可以包含 `initialization` 和 `finalization` 代码片段. 其中的部分分别规定了程序开始执行和结束时要执行的代码。
...
一个unit 可以引入另一个unit. 单元文件可以在 interface 中引入, 也可以在 implementation 引入. 在interface 中引入你还可以在其他unit 的代码前定义新的公有的(public)代码片段(procedures, types...). 在implementation 中引入则有限制性，只允许你在implementation 中使用引入的unit中的标识符。
...
_在interface代码段中循环引入unit_ 是被禁止的. 也就是说两个 unit 不能在各自的interface片段中同时相互引用对方.
//所有标识符使用前必须先被声明.
原因在于编译器要先"理解"所有的在interface 中被引用的unit，这样Pascal 编译器就能快速地完成编译并且检测到 _哪些unit 需要被重新编译_ 。这样就无需借助 ``Makefile`` 即可完成简单的项目的编译任务，也可以保证所有的依赖项都能被正确地更新，而无需 _重新编译所有文件_.
...
_Published properties_ are the basis of a _serialization_ (also known as _streaming components_) in Pascal.
